...
router.post('/addDelivery', (req, res) => {
    let { postid, price, ecp, address, order_id } = req.body
    if (!(postid && price && ecp && address && order_id)) {
        res.json({
            code: 401,
            message: '入参不符'
        })
    ...
    let sql = "insert into t_delivery (postid,price,ecp,address,order_id,create_time) values (?,?,?,?,?,?)";
    let param = [postid, price, ecp, address, order_id, Date.now()];
